...
//   var allElements = document.getElementsByTagName('include');
	...
	set:{
		...
		menu:{
			...
			linkStat: function () {
				...
				var thisPg = decodeURI( "../.."+window.location.pathname.replace("/kbstar/","/").replace("/ui/","/").replace("/static/","/").replace("/mo/","/").replace("/pc/","/").replace("/admin/","/") );
				console.log(thisPg);
			...
			togDev:function(){ // F4 키
				...
				if(tPort == "9084" || tName == "localhost" || tName == "127.0.0.1"){
					console.log(tUrl.replace(tHost,"kbstar.devtree.co.kr"));
					location.href = tUrl.replace(tHost,"kbstar.devtree.co.kr");
				}
				if(tOrg == 'http://kbstar.devtree.co.kr' || tOrg == 'http://10.27.225.22:9085'){
